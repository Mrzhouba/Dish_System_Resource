...
class Initial_Work_Page(QMainWindow, Ui_Initial_Form):
    ...
    def show_wrenchPlot_status(self, msg):
        if msg == 1:
            self.Initial_messgae_txt.append(str(self.timedisplay) + ": 力矩监控关闭")


    def save_initial_log(self):
        if len(self.save_initial_txt_path) == 0:
            self.save_initial_txt_path = QFileDialog.getSaveFileName(self, "设置路径","/home/robot/photo","All Files (*);;Text Files (*.txt)")
            self.Initial_messgae_txt.append('选择文件路径为' + str(self.save_initial_txt_path[0]))
        else:
    ...
    def show_need_moveDis(self, msg):
        if msg == 0:
            ...
            if self.power_switch.text() == 'power_off':
                ...
                QMessageBox.information(self, "提示", "小车未上电", QMessageBox.Yes)
...
class Generate_Menu_Page(QMainWindow, Ui_Menu_Form):
    ...
    def recognize_number(self):
        if self.dish_location >= 0:
            ...
            if food_name != "None":
                self.menu_message_text.append(str(self.timedisplay) + ' 标签: ' + str(food_name))
                self.manual_menu[food_name] = self.dish_location
            else:
                self.menu_message_text.append(str(self.timedisplay) + ' 标签: 未识别到菜品')
        else:
    ...
    #             str(self.timedisplay) + '：标签 间隔 ' + str(self.move_interval_num) + ', 次数 ' + str(self.move_frequency_num))
    ...
    def choose_save_menu_path(self):
        path = QFileDialog.getSaveFileName(self, "设置路径","/home/robot","All Files (*);;Text Files (*.txt)")
...
class Dig_Dish_Page(QMainWindow, Ui_Dig_dish_Form):
    ...
    def ur3_add_bowl(self):
        reply = QMessageBox.information(self, "提示", "即将开始补碗, 请确定小车处于初始位置",
                                        QMessageBox.Yes | QMessageBox.No, QMessageBox.No)
        if reply == QMessageBox.Yes:
            self.ur3_add_bowl_thread.start()
        else:
            self.order_list_text.append(str(self.timedisplay) + ': 取消补碗')

    def show_add_bowl_finish(self):
        self.order_list_text.append(str(self.timedisplay) + ': 已补充5个碗')
    ...
    def button_control_dig_work(self, msg):
        if self.system_status_variable:
            if msg == 0:
                ...
                if self.dig_dish_start_button.isEnabled():
                    QMessageBox.information(self, "提示", "打菜工作未开始", QMessageBox.Ok)
                else:
                    self.dig_stop_work()
            elif msg == 2:
                if self.dig_dish_start_button.isEnabled():
            ...
            QMessageBox.information(self, "提示", "打菜系统未初始化", QMessageBox.Ok)


    def dig_dish_start_work(self, manual_order):
        if len(manual_order) == 1:
            reply = QMessageBox.information(self, "提示", "当前碗点数量为" + str(manual_order[0]) + "个, 确定开始打菜吗？",
                                            QMessageBox.Yes | QMessageBox.No, QMessageBox.No)
            if reply == QMessageBox.Yes:
                self.dig_dish_thread.receive_choose_dish_oeder(manual_order)
                self.dig_dish_thread.start()
            else:
                self.order_list_text.append(str(self.timedisplay) + "取消打菜")
    ...
    def dig_dish_status_feedback(self, msg):
        if msg == 1:
            ...
            self.order_list_text.append(str(self.timedisplay) + ": 添加菜品")
    ...
    def closeEvent(self, event):
        ...
        if reply == QMessageBox.Yes:
            if self.initial_work_class.sys_end.isEnabled():
                QMessageBox.information(self, '提示', '系统未关闭', QMessageBox.Yes)
                event.ignore()
            else:
                event.accept()
        else:
